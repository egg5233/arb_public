...
11. **Binance delist filter** (all scan types): Background goroutine polls Binance delist announcements API every 6h, parses coin names from titles, stores blacklist in Redis (`arb:delist:{SYMBOL}`) with TTL until delist date + 7 days. Rejects any opportunity with a blacklisted symbol. Engine auto-exits active positions with Binance legs via emergency market close + dashboard alert.
