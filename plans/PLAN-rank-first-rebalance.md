...
- v28 (2026-04-20): Fixed stop-condition + 2 stale symmetric-reservation anchors in conservative-stance/Out-of-Scope prose.
- v29 (this draft): Codex INDEPENDENT re-review of v28 (task d331e3c7 at pinned HEAD 9642b4db) found 2 residuals: (1) Test 11 scenario text still cited stale anchors `allocator.go:181-200 / :936-940` — missed during v27/v28 sweeps because Test 11 lives in a separate test-scenarios section. v29 updates Test 11 to the corrected anchors `:142-150 / :855-857`. (2) Rollout section still said "next sequence after 5bugs + margin-unify merge" and "after both dependencies merge", contradicting v27/v28's two-option guidance (margin-ratio-unify is optional because plan works in current `marginRatio` regime too). v29 rewrites rollout to say "after 5bugs v5 and whichever margin-gating regime is chosen for implementation".
...
**MUST land AFTER:**
1. `plans/PLAN-rebalance-partial-5bugs.md` v5 — provides `selectedChoices`, `buildChoices`, dry-run prune, override store, `localTransferHappened`. Present on pinned branch `plan/sirenusdt-3bug-fix` at HEAD 4fdc9f26.
2. A margin-ratio-unify implementation (plan + code) that introduces `donorHasHeadroom` (and preserves `capByMarginHealth`). At pinned branch HEAD 4fdc9f26, no such plan file is present — `plans/PLAN-margin-ratio-unify.md` does not exist in the checked-out tree, and the actual donor-health gate is still `bal.marginRatio >= e.cfg.MarginL4Threshold`. Implementers have two options: (a) land the margin-ratio-unify dependency first (creating its plan, running its own review loop, and merging `donorHasHeadroom`), then re-verify this plan's extracted helper body still matches the inline block at implementation time; OR (b) keep the current `marginRatio` check in both the inline block and the extracted helper — no new donor-gate semantics are introduced by this plan regardless. Either way, the parity test (`allocator_transferable_parity_test.go`) guarantees the extracted helper and the inline block stay identical. The plan works cleanly in both regimes.
...
for i := range attempts {
    att := attempts[i]
    if att.alt != nil {
        // Mirror pool allocator's pre-approval alt guards (allocator.go:553-564).
        ...
        // (empty = passed). Build altOpp mirroring allocator.go:517-524.
...
**Note on `e.discovery.CheckPairFilters`**: the method exists as `Scanner.CheckPairFilters(opp models.Opportunity) string` at `internal/discovery/scanner.go:776-780`, returning a rejection reason (empty string = passed). The pool allocator uses it with the same altOpp-building pattern at `allocator.go:517-524`.
...
(Note: if a FUTURE plan (not the pinned 5bugs v5 plan reviewed here — 5bugs v5 itself uses 3-arg / `FundedReceivers` only) extends `rebalanceExecutionResult` with a pending-deposit field and extends `keepFundedChoices` with a fourth pending argument, update both call sites to pass that new field. The current pushed-branch source and pinned 5bugs v5 dependency are 3-arg / `FundedReceivers` only.)
...
- **Executor/cross path** (`engine.go:1099-1106`): executor ran → `result.FundedReceivers` (`map[string]float64`) carries the funded-receiver signal that `keepFundedChoices` consumes → `keepFundedChoices(buildChoices(selectedChoices), result.PostBalances, result.FundedReceivers)`. `rebalanceExecutionResult` has `FundedReceivers` but no `PendingDeposits` field at pushed HEAD. If a future plan (not the pinned 5bugs v5 plan reviewed here) extends `rebalanceExecutionResult` with a pending-deposit field and extends `keepFundedChoices` with a fourth pending argument, update both call sites to pass that new field. The current pushed-branch source and pinned 5bugs v5 dependency are 3-arg / `FundedReceivers` only. For an alt pair that fit under pre-state balances and needed no transfer, `result.PostBalances == balances`, `FundedReceivers` is empty, so the alt choice is retained.
...
- **Version**: next sequence after 5bugs v5 and whichever margin-gating regime is chosen for implementation. Current `VERSION` file at repo root reads `0.32.27`; this plan ships as the next version after the required 5bugs dependency and any separately-landed margin-ratio-unify work, if that option is chosen. Implementer bumps both `VERSION` and `CHANGELOG.md` at commit time.
