...
| `internal/engine/allocator.go` | Extract inline `transferable` builder (lines 348-381) into new `func (e *Engine) buildTransferableCache(balances map[string]rebalanceBalanceInfo) *risk.PrefetchCache`. `runPoolAllocator` calls the helper AND the downstream `revalCache` at `:430-432` is updated to reuse the same cache (see Change 3 details). |
...
The `keepFundedChoices` helper is provided by 5bugs (dependency); its contract is "drop any choice whose required margin exceeds the post-state per-leg balance, accounting for funded receivers and pending deposits". Real signature at `allocator.go:246`:
