...
- **Rebalance filter parity** — rebalanceScan now applies 5 entry filters (persistence, volatility, cooldown, interval, backtest) but excludes funding window filter so funds are prepared before entry window opens (`scanner.go`)
